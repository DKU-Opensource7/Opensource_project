...
class NoFakeShield:
    ...
    def inverse_warp(self, warped_img, M, original_shape):
        M_inv = cv2.invertAffineTransform(M)
        unwarped = cv2.warpAffine(warped_img, M_inv, (original_shape[1], original_shape[0]), borderValue=(0,0,0))
        
        # 마스크 생성 (테두리 부드럽게 처리)
        mask_small = np.full((112, 112, 3), 255, dtype=np.float32)
        border = 15
        mask_small[:border, :] = 0
        mask_small[-border:, :] = 0
        mask_small[:, :border] = 0
        mask_small[:, -border:] = 0
        mask_small = cv2.GaussianBlur(mask_small, (31, 31), 0)
        
        mask_warped = cv2.warpAffine(mask_small, M_inv, (original_shape[1], original_shape[0]), borderValue=(0,0,0))
        mask_warped = mask_warped[:,:,0] / 255.0
        return unwarped, mask_warped
    ...
    def generate_noise(self, image_path, epsilon=0.05, alpha=0.01, iterations=30, lambda_lpips=20.0):
        ...
        print(f"🧩 [Process] Found {len(faces)} face(s). Generating noise...")
        ...
        for i, face in enumerate(faces):
            ...
            noise = delta.detach().cpu()
            noisy_face_tensor = face_tensor.cpu() + noise
            noisy_face_img = (noisy_face_tensor.squeeze(0).permute(1, 2, 0).numpy() * 0.5 + 0.5) * 255.0
            noisy_face_img = np.clip(noisy_face_img, 0, 255).astype(np.uint8)

            unwarped_noisy, mask_alpha = self.inverse_warp(noisy_face_img, M, img_rgb.shape)
            unwarped_noisy_float = unwarped_noisy.astype(np.float32) / 255.0
            
            result_image_float = (1 - mask_alpha[:, :, np.newaxis]) * result_image_float + \
                                 mask_alpha[:, :, np.newaxis] * unwarped_noisy_float

        return cv2.cvtColor(np.clip(result_image_float * 255, 0, 255).astype(np.uint8), cv2.COLOR_RGB2BGR)
